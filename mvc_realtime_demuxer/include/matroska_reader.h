...
public:
    ...
    // Optional trackNumber to filter seeking (prevents stopping on audio blocks)
    ...
    // Provide an external duration hint (milliseconds) when the MKV header lacks Duration.
    void setExternalDurationMs(int64_t durationMs);

    // When a seek fails to find an IDR (EOF), try rewinding a few seconds and re-seek.
    bool rewind_after_failed_seek(int64_t timestampMs, uint32_t ms_backoff = 5000);
