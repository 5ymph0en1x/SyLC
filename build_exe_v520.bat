@echo off
REM === SyLC 3D Player v5.2.0 - standalone no-console build (Nuitka, one-folder) ===
...
REM v5.2.0 ships NO depth models. The twenty ONNX graphs (4.61 GB) live in the
...
uv run python -m nuitka SyLC_3D_Player.py ^
  ...
  --output-dir=build_release_v520 ^
  --output-filename=SyLC_3D_Player.exe ^
  --company-name=SyLC --product-name="SyLC 3D Player" --file-version=5.2.0 --product-version=5.2.0
...
xcopy /E /I /Y tools build_release_v520\SyLC_3D_Player.dist\tools >nul

echo.
echo Build done. Result in build_release_v520\SyLC_3D_Player.dist
