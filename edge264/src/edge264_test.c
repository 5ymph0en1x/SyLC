	...
	if (!quit) {
		...
		do {
			...
			while (!edge264_get_frame(d, &out, 0)) {
				// DUMP first 30 frames' Y plane stats to verify decode quality
				if (dumped_frame < 30) {
					int w = out.width_Y, h = out.height_Y, sY = out.stride_Y;
					unsigned long long sum = 0;
					unsigned mn = 255, mx = 0;
					for (int y = 0; y < h; y++) {
						const uint8_t *row = out.samples[0] + y * sY;
						for (int x = 0; x < w; x++) {
							unsigned p = row[x];
							sum += p;
							if (p < mn) mn = p;
					...
					fprintf(stderr, "[Y-STATS frame %d] w=%d h=%d stride=%d avg=%.2f min=%u max=%u FrameId=%u POC=%d\n",
						dumped_frame, w, h, sY, avg, mn, mx, out.FrameId, out.PictureOrderCnt);
					// also dump first 16 Y values of the first row
					fprintf(stderr, "[Y-STATS frame %d] Y[0..15]:", dumped_frame);
					for (int x = 0; x < 16; x++) fprintf(stderr, " %02x", out.samples[0][x]);
					fprintf(stderr, "\n");
					if (out.samples_mvc[0]) {
						// MVC: dump mvc Y stats too
						unsigned long long sum2 = 0;
						unsigned mn2 = 255, mx2 = 0;
						for (int y = 0; y < h; y++) {
							const uint8_t *row = out.samples_mvc[0] + y * sY;
							for (int x = 0; x < w; x++) {
								unsigned p = row[x];
								sum2 += p;
								if (p < mn2) mn2 = p;
						...
						double avg2 = (double)sum2 / ((double)w * h);
						fprintf(stderr, "[Y-STATS frame %d MVC] avg=%.2f min=%u max=%u FrameId_mvc=%u POC_mvc=%d\n",
							dumped_frame, avg2, mn2, mx2, out.FrameId_mvc, out.PictureOrderCnt_mvc);
					}
					fflush(stderr);
					dumped_frame++;
				}
