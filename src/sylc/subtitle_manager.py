...
from sylc.pgs_subtitle_parser import PGSSubtitleParser, PGSDisplaySet
...
class SubtitleManager(QObject):
    ...
    def load_subtitle_file(self, filepath: str) -> bool:
        ...
        if success:
            self.install_parser(parser, filepath)
        else:
    ...
    def install_parser(self, parser: PGSSubtitleParser, source: str = '<parsed>') -> bool:
        """Adopt a fully parsed PGS document on the manager's owning thread.

        Parsing can take place in a worker, but this QObject's live playback
        state must only be replaced by the main thread after the media-session
        token has been checked.
        """
        if not isinstance(parser, PGSSubtitleParser):
            return False
        self._parser = parser
        ...
        logger.info(
            "[SubtitleManager] Installed %d display sets from %s",
            len(parser.display_sets), source)
        return True
